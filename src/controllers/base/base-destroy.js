 ...
 */
const baseDestroy = async (model, id, options = {}) => { // Validate model exists
    if (! prisma[model]) {
    ...
    return await prisma.$transaction(async (tx) => {
        try { // Check if record exists
            ...
            if (options.softDelete && record.deletedAt === null) {
                if (verbose) 
                    console.log(`Soft deleting ${model} ${idField}=${id}`);
            ...
            // Perform hard delete
            if (options.verbose) 
                console.log(`Deleting ${model} ${idField}=${id}`);
