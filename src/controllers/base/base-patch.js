...
const basePatch = async (model, id, action, options = {}) => {
    ...
    return await prisma.$transaction(async (tx) => {
        try { // Determine new status value
            let statusUpdate;
            switch (action.toLowerCase()) {
                case 'remove': statusUpdate = 'inactive';
                    break;
                case 'restore': statusUpdate = 'active';
                    break;
                default:
                    throw new Error(`Invalid action "${action}". Use 'remove' or 'restore'.`);
            }
