 ...
 */
const baseUpdate = async (model, id, data, file, uploadPath, options = {}) => { // Validate model exists
    if (! prisma[model]) {
        ...
        try { // Convert and validate data
            ...
            if (file && uploadPath) {
                ...
                if (existingRecord[pictureField]) {
                    const oldFilePath = path.join(uploadPath, existingRecord[pictureField]);
                    try {
