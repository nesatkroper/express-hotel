...
const convertData = (data, modelSchema) => {
    try {
        ...
        for (const key in data) {
            if (!modelSchema[key]) {
                console.warn(`Warning: Key "${key}" is not defined in schema for this model.`);
                continue;
            }

            if (modelSchema[key] === "Int") {
                convertedData[key] = isNaN(parseInt(data[key])) ? null : parseInt(data[key]);
            } else if (modelSchema[key] === "Float") {
                convertedData[key] = isNaN(parseFloat(data[key])) ? null : parseFloat(data[key]);
